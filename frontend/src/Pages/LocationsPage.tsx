...
export default function LocationsPage() {
  const content: JSX.Element[] = locations.map((location) => (
    <Col key={location.id} xs={24} sm={12} md={8} lg={6} xl={4}>
      <Card title={location.name} hoverable>
        <Text>{location.address}</Text>
        <br />
        <Text type="secondary">Canchas: {location.courts}</Text>
        <br />
        {location.indoor ? <Tag color="green">Techada</Tag> : <Tag color="blue">Outdoor</Tag>}
      </Card>
    </Col>
  ));
  return (
